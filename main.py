...
def fl_finetune(
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        elif 'data_wiz' in data_path or data_path == './data_mix/20':  # FIXED: Support any data_wiz path
            full_prompt = prompter.generate_prompt(
                data_point["instruction"],
                None,
                data_point["output"],
    ...
    if full == False:
        if stacking == False:
            if zero_padding:
                # FIXED: Added PEFT wrapper for zero_padding mode
                config = LoraConfig(
        ...
        for client_id in selected_clients_set:
            if full == False:
                if Adalora:
                    ...
                    model_client = get_peft_model(model, config)  # FIXED: Use 'model' not 'model_client'
                else:
                    if stacking:
                        if heter:
                            config = LoraConfig(
